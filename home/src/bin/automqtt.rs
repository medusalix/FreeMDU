...
/*
    ...
    */
    ...
    // 1. Launch network tasks in the background (ONLY ONCE, HERE!)
    spawner.spawn(network_stack_task(net_runner).unwrap());
    spawner.spawn(wifi_connect_task(wifi_controller).unwrap());

    // 2. Wait until DHCP assigns the network configuration
    ...
    // Initialize the static cell to get a &'static str reference from it
    let mqtt_host_static: &'static String = MQTT_HOST_CELL.init(mqtt_host_string);

    // 4. Build the MQTT client with the static IP reference
    ...
    // 5. Launch the tasks responsible for MQTT as well
