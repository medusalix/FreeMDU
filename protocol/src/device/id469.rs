...
const PROP_LOAD_SENSOR_WEIGHT: Property = Property {
    kind: PropertyKind::Io,
    id: "load_sensor_weight",
    name: "Load Sensor Weight",
    unit: None,
};
...
impl<P: Read + Write> WashingMachine<P> {
    ...
    pub async fn query_program_selector(&mut self) -> Result<SelectorPosition, P::Error> {
        ...
        SelectorPosition::from_repr(self.intf.read_memory(0x00b6).await?)
            .ok_or(Error::UnexpectedMemoryValue)
    }
    ...
    pub async fn query_tachometer_speed(&mut self) -> Result<(u16, u16), P::Error> {
        let current: u16 = self.intf.read_memory(0x00d3).await?;
        let target: u16 = self.intf.read_memory(0x018b).await?;

        Ok((current / 10, target))
    }
...
impl<P: Read + Write> Device<P> for WashingMachine<P> {
    ...
    async fn query_property(&mut self, prop: &Property) -> Result<Value, P::Error> {
        match *prop {
            ...
            PROP_LOAD_SENSOR_WEIGHT => {
                Ok(self.query_load_sensor_weight().await?.to_string().into())
            }
