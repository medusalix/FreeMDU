...
//!
//! Supports appliances with software ID 2895, which typically use an EDPL 151-B board or similar.
//!
...
/// use freemdu::device::{Device, id2895::WashingMachine};
///
...
impl<P: Read + Write> WashingMachine<P> {
    ...
    pub async fn query_fault(&mut self, code: FaultCode) -> Result<Fault, P::Error> {
        let ((active_addr, active_mask), (stored_addr, stored_mask)) = match code {
            FaultCode::PressureSensor => ((0x0072, 0x01), (0x004e, 0x01)),
            FaultCode::NtcThermistor => ((0x00cb, 0x20), (0x004e, 0x02)),
            FaultCode::Heater => ((0x007c, 0x40), (0x004e, 0x04)),
            FaultCode::Tachometer => ((0x0073, 0x20), (0x004e, 0x08)),
            FaultCode::DetergentOverdose => ((0x0044, 0x08), (0x004e, 0x10)),
            FaultCode::WaterInlet => ((0x0044, 0x20), (0x004e, 0x20)),
            FaultCode::Drainage => ((0x0044, 0x40), (0x004e, 0x40)),
            FaultCode::FinalSpinSpeed => ((0x0078, 0x01), (0x004e, 0x80)),
            FaultCode::Eeprom => ((0x004f, 0xd0), (0x004f, 0x01)),
        };
    ...
    pub async fn query_program_type(&mut self) -> Result<ProgramType, P::Error> {
        // Program types are defined in a lookup table at address 0x2f93.
    ...
    pub async fn query_program_temperature(&mut self) -> Result<u8, P::Error> {
        // Program temperatures are defined in a lookup table at address 0x2fbf.
    ...
    pub async fn query_program_spin_speed(&mut self) -> Result<u16, P::Error> {
        ...
        // and the machine's programming configuration at 0x021b in the subroutine at 0x38d4.
    ...
    pub async fn query_program_phase(&mut self) -> Result<ProgramPhase, P::Error> {
        // Program phases are defined in a lookup table at address 0xa0a1.
    ...
    pub async fn query_ntc_resistance(&mut self) -> Result<u32, P::Error> {
        let val: u8 = self.intf.read_memory(0x01be).await?;
    ...
    pub async fn query_temperature(&mut self) -> Result<(u8, u8), P::Error> {
        // Temperatures are defined in a lookup table at address 0x8f09.
    ...
    pub async fn query_water_level(&mut self) -> Result<(u8, u8), P::Error> {
        // Target water levels are defined in a lookup table at address 0x9ca9.
        ...
        // In that case, the target is set from the lookup table at address 0xa891.
    ...
    pub async fn start_program(&mut self) -> Result<(), P::Error> {
        // Programs are managed by a state machine subroutine at 0x3711.
