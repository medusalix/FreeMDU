...
//!
//! Supports appliances with software ID 218, which typically use an EDPW 213 board or similar.
//!
...
/// use freemdu::device::{Device, id218::WashingMachine};
///
...
impl<P: Read + Write> WashingMachine<P> {
    ...
    pub async fn query_fault(&mut self, code: FaultCode) -> Result<Fault, P::Error> {
        let ((active_addr, active_mask), (stored_addr, stored_mask)) = match code {
            FaultCode::PressureSensor => ((0x002f, 0x01), (0x000e, 0x01)),
            FaultCode::NtcThermistor => ((0x0085, 0x20), (0x000e, 0x02)),
            FaultCode::Heater => ((0x0039, 0x40), (0x000e, 0x04)),
            FaultCode::Tachometer => ((0x0030, 0x20), (0x000e, 0x08)),
            ...
            FaultCode::FinalSpinSpeed => ((0x0035, 0x01), (0x000e, 0x80)),
    ...
    pub async fn query_operating_state(&mut self) -> Result<OperatingState, P::Error> {
        OperatingState::from_repr(self.intf.read_memory(0x008a).await?)
    ...
    pub async fn query_selected_program(&mut self) -> Result<Program, P::Error> {
        ...
        Program::from_repr(self.intf.read_memory(0x0072).await?).ok_or(Error::UnexpectedMemoryValue)
    }
    ...
    pub async fn query_program_type(&mut self) -> Result<ProgramType, P::Error> {
        // Program types are defined in a lookup table at address 0xa760.
        // The current type is determined by reading the value at 0x0001
        // to index into this table.
        ProgramType::from_repr(self.intf.read_memory(0x009d).await?)
            .ok_or(Error::UnexpectedMemoryValue)
    }
    ...
    pub async fn query_program_temperature(&mut self) -> Result<u8, P::Error> {
        // Program temperatures are defined in a lookup table at address 0xa776.
        // The current temperature is determined by reading the value at 0x0001
        // to index into this table.
        Ok(self.intf.read_memory(0x009e).await?)
    }
    ...
    pub async fn query_program_options(&mut self) -> Result<ProgramOption, P::Error> {
        // The options are used to set the front panel indicator lights at 0x006c.
    ...
    pub async fn query_program_spin_setting(&mut self) -> Result<SpinSetting, P::Error> {
        // The spin setting is used to set the front panel indicator lights at 0x0066.
    ...
    pub async fn query_program_spin_speed(&mut self) -> Result<u16, P::Error> {
        ...
        let speed: u8 = self.intf.read_memory(0x00a9).await?;
    ...
    pub async fn query_program_phase(&mut self) -> Result<ProgramPhase, P::Error> {
        // Program phases are defined in a lookup table at address 0xea99.
        ...
        // This value is used to set the front panel indicator lights at 0x0069.
        ProgramPhase::from_repr(self.intf.read_memory(0x005f).await?)
            .ok_or(Error::UnexpectedMemoryValue)
    }
    ...
    pub async fn query_display_contents(&mut self) -> Result<String, P::Error> {
        let display: [u8; 4] = self.intf.read_memory(0x005b).await?;
    ...
    pub async fn query_ntc_resistance(&mut self) -> Result<u32, P::Error> {
        let val: u8 = self.intf.read_memory(0x018e).await?;
    ...
    pub async fn query_temperature(&mut self) -> Result<(u8, u8), P::Error> {
        // Temperatures are defined in a lookup table at address 0xde42.
    ...
    pub async fn query_water_level(&mut self) -> Result<(u8, u8), P::Error> {
        // Target water levels are defined in a lookup table at address 0xe751.
        ...
        // In that case, the target is set from the lookup table at address 0xf15a.
    ...
    pub async fn query_motor_pwm_duty_cycle(&mut self) -> Result<u8, P::Error> {
        // Read directly from the PWM register PWML.
        let duty: u8 = self.intf.read_memory(0x00eb).await?;
    ...
    pub async fn query_tachometer_speed(&mut self) -> Result<(u16, u16), P::Error> {
        // The target speed is selected from a lookup table at 0xe200,
        // based on the value of the memory at 0x020b multiplied by 2.
        // PWM calculations for the motor are performed in a subroutine at 0xb911.
        let speed: [u8; 5] = self.intf.read_memory(0x0092).await?;
    ...
    pub async fn start_program(&mut self) -> Result<(), P::Error> {
        // Programs are managed by a state machine subroutine at 0xac30.
        // The current state is stored at 0x00a6. Known state values include:
        ...
        let state: u8 = self.intf.read_memory(0x00a6).await?;

        if state == 0x01 {
            Ok(self.intf.write_memory(0x00a6, 0x02u8).await?)
        } else {
