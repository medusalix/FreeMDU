...
impl<P: Read + Write> WashingMachine<P> {
    ...
    pub async fn query_active_actuators(&mut self) -> Result<Actuator, P::Error> {
        ...
        let actuators: u16 = self.intf.read_memory(0x0039).await?;

        // Ignore the actuator at bit 8 for now. Its purpose is not known at this time.
        Actuator::from_bits(actuators & !0x0100).ok_or(Error::UnexpectedMemoryValue)
    }
